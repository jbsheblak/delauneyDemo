...
namespace NDelaunay
{
   ...
   struct SFloat3
   {
      ...
      explicit SFloat3(float _x, float _y, float _z)
         : x(_x)
         , y(_y)
         , z(_z)
      {
   ...
   float circle_test_det(SFloat2 const & a, SFloat2 const & b, SFloat2 const & c, SFloat2 const & q)
   {
      ...
      float const d0 = det3(SFloat3(b.x, b.y, b2), SFloat3(c.x, c.y, c2), SFloat3(q.x, q.y, q2));

      float const d1 = det3(SFloat3(a.x, a.y, a2), SFloat3(c.x, c.y, c2), SFloat3(q.x, q.y, q2));

      float const d2 = det3(SFloat3(a.x, a.y, a2), SFloat3(b.x, b.y, b2), SFloat3(q.x, q.y, q2));

      float const d3 = det3(SFloat3(a.x, a.y, a2), SFloat3(b.x, b.y, b2), SFloat3(c.x, c.y, c2));
   ...
   struct SEdge
   {
      ...
      bool operator==(SEdge const & other) const { return (mV0 == other.mV0) && (mV1 == other.mV1); }

      bool operator!=(SEdge const & other) const { return !(*this == other); }
   ...
   struct STriPair
   {
      ...
      bool HasPairedTriangles() const { return (mTri0 != skInvalidIndex) && (mTri1 != skInvalidIndex); }
   ...
   public:
      ...
      SFloat2 const * Get(uint32_t const idx) const { return reinterpret_cast<SFloat2 const *>(mpPositions8 + intptr_t(idx) * mPositionStride); }
