...
namespace NDemo
{
   namespace
   {
      ...
      bool build_shader(std::string const & path, SShader & shader)
      {
         ...
         if ( FAILED(gpDevice->CreateVertexShader(shader.mpVSShaderBlob->GetBufferPointer(),
                                                  shader.mpVSShaderBlob->GetBufferSize(),
                                                  nullptr /*pClassLinkage*/,
                                                  shader.mpVSShader.GetAddressOf())) )
         {
            return false;
         }

         if ( FAILED(gpDevice->CreatePixelShader(shader.mpPSShaderBlob->GetBufferPointer(),
                                                 shader.mpPSShaderBlob->GetBufferSize(),
                                                 nullptr /*pClassLinkage*/,
                                                 shader.mpPSShader.GetAddressOf())) )
         {
            return false;
         }
      ...
      {
         rebuild_index_buffer_from(data.mMesh, data.mShowDelaunayMesh ? data.mMesh.mTriangulatedIndices : data.mMesh.mIndices);
      }
   ...
   void render()
   {
      ...
      // draw
      {
         ...
         gpDeviceCtx->IASetVertexBuffers(0 /*startSlot*/,
                                         1 /*numBuffers*/,
                                         sData.mMesh.mpVertexBuffer.GetAddressOf(),
                                         &skPositionColorStride,
                                         &skPositionColorOffset);
